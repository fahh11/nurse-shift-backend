...
export const createShiftTemplate = async(
    ...
    const results: CreateShiftTemplateOutputDto[] = []

    for (const item of input) {
        // หา ward ที่ต้องการสร้าง shift_template
        const wardData = await repos.wardRepo.findById(item.wardId)
        if (!wardData) {
        ...
        const createdShiftTemplate = await repos.shiftTemplateRepo.create(newShiftTemplate)

        // create shift requirement
        const newShiftRequirement = new ShiftRequirement({
            requiredPeople: item.requiredPeople,
            shiftTemplateId: createdShiftTemplate.shiftTemplateId,
            effectiveFrom: new Date(),
            effectiveTo: null,
        })
        const createdShiftRequirement = await repos.shiftRequirementRepo.create(newShiftRequirement)

        results.push({
            shiftTemplate: createdShiftTemplate,
            shiftRequirement: createdShiftRequirement
        })
    }
